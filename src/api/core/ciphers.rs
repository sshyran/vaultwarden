...
async fn sync(data: SyncData, headers: Headers, mut conn: DbConn) -> Json<Value> {
    ...
    let cipher_sync_data = CipherSyncData::new(&headers.user.uuid, &ciphers, &mut conn).await;
...
impl CipherSyncData {
    pub async fn new(user_uuid: &str, ciphers: &[Cipher], conn: &mut DbConn) -> Self {
        ...
        let cipher_folders: HashMap<String, String> =
            FolderCipher::find_by_user(user_uuid, conn).await.into_iter().collect();

        // Generate a HashSet of all the Cipher UUID's which are marked as favorite
        let cipher_favorites: HashSet<String> =
            Favorite::get_all_cipher_uuid_by_user(user_uuid, conn).await.into_iter().collect();
